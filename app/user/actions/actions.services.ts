...
export class ActionsServices {
	...
	public async addToLike(userData: any, movieId: string) {
		...
		if (existingUser) {
			const likesData = existingUser.liked_movies
				? existingUser.liked_movies
				: [];
			if (movieData) {
				if (!includes(likesData, movieId)) {
					...
					await userModel.updateOne(
						...
						{ liked_movies: likesData },
					);
					return {
						...this.actionsFactory.buildTokenResponse(existingUser),
						liked_movies: likesData,
					};
				} else {
	...
	public async unlikeMovie(userData: any, movieId: string) {
		...
		const existingUser = await userModel
			...
			.lean();
		if (existingUser) {
			let likesData = existingUser.liked_movies
				? existingUser.liked_movies
				: [];
			if (!includes(likesData, movieId)) {
