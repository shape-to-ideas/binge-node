...
export class MovieServices {
	...
	constructor(
		...
		@inject(Symbols.CounterServices)
	...
	public async getMovies(ids: number[]) {
		...
		return map(movieResponse, (result) =>
			this.movieFactory.buildMovies(result),
		);
	}
	...
	public async searchByTitle(keyword: string) {
		...
		return map(response.data.results, (result) =>
			this.movieFactory.buildMovies(result),
		);
	}
	...
	public async saveMoviesToDB(moviesData: Movies[]) {
		...
		const movieInDb = await movieModel
			.find({ id: { $in: movieIds } })
			.lean();
		const moviesToInsert = filter(moviesData, (movie) =>
			isEmpty(find(movieInDb, { id: movie.id })),
		);
	...
	public async insertMoviesByPopularity() {
		try {
			let pageNumber;
			const counter = await this.counterServices.getTopRatedPageCount({
				name: 'topRatedPageCount',
			});
			if (counter.length == 0) {
				await this.counterServices.insertTopRatedCounter({
					name: 'topRatedPageCount',
					count: 1,
				});
				pageNumber = 1;
				...
				await this.counterServices.updateTopRatedCounter(
					{ name: 'topRatedPageCount' },
					{ count: nextCounter },
				);
			}

			const apiResponse = await this.tmdbServices.getTopRatedMovies({
				page: pageNumber,
			});
			const moviesList = get(apiResponse, 'data.results');
			await this.saveMoviesToDB(moviesList);
			this.loggerServices
				.logger(this.topRatedSchedulerLogPath)
				.info(`successfully inserted page number ${pageNumber}`);
		} catch (e) {
			this.loggerServices.logError(e);
		}
	}

	public async getMovieByGenre(
		genreId: number,
		page: number,
		pageSize: number,
	) {
		...
		const movies = await movieModel.aggregate([
			{ $match: { genre_ids: genreId } },
			{
				$addFields: {
					vote_by_rating: {
						$cond: {
							if: { $gt: ['$vote_average', 0] },
							then: { $divide: ['$vote_count', '$vote_average'] },
							else: 10,
						},
					},
				},
